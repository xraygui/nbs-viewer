...
:mod:`request` is the frozen ``PlotRequest`` and the two builders that turn
something a user drew into one. :mod:`plan` turns a request into the storage
indices to read, and is the only place a load is narrowed. :mod:`stages` is
the uniformly ``DataArray -> DataArray`` set that runs between the read and
the pack.
