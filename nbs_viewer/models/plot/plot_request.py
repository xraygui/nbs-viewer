...
def plane_storage_axes(
    view: ViewSpec,
    plane_frame: Optional[PlotViewFrame] = None,
) -> Optional[Tuple[int, int]]:
    """
    Resolve the storage axes carrying plot Y and plot X.
    ...
    view : ViewSpec
        Rank-bound view.
    plane_frame : PlotViewFrame, optional
        Parent plane frame, used only for a 2-D key whose own view has been
        reduced to a profile.

    Returns
    -------
    tuple of int or None
        ``(plot_y_axis, plot_x_axis)``, or None when there is no plane.
    """
    order = view.plot_axis_order()
    if view.plot_ndim == 2 and len(order) >= 2:
        return order[-2], order[-1]
    if view.ndim == 2 and plane_frame is not None:
        return plane_frame.plot_y_dim, plane_frame.plot_x_dim
    return None
...
def plan_fetch(
    ...
    *,
    ...
    replaced cannot disagree about orientation again.
    ...
    plane_axes : tuple of int, optional
        Storage axes of the parent plot plane. Needed when the request's own
        view has already been rewritten to a 1-D profile, which loses them.
    ...
    if plane_axes is None:
        plane_axes = plane_storage_axes(view, plane_frame)
    ...
    if plane_axes is None:
        raise ValueError("cannot resolve plot-plane storage axes for an ROI fetch")
