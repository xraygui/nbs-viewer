...
def test_to_view_crop_maps_the_drawn_box_back_to_storage():
    full_frame = _make_full_frame(y_count=5, x_count=6)
    region = RectRegion(x0=1.5, x1=3.5, y0=0.5, y1=2.5)

    crop = region.to_view_crop(full_frame, (0, 1))
...
def test_storage_bbox_inverts_row_flip_for_bottom_roi():
    ...
    crop = region.to_view_crop(frame, (0, 1))
...
def test_roi_under_a_crop_loads_the_intersection_and_a_matching_frame():
    ...
    crop = RectRegion(
        x0=581.93, y0=156.16, x1=1478.57, y1=899.24
    ).to_view_crop(full_frame, (2, 3))
    roi = RectRegion(x0=700, y0=200, x1=1200, y1=700)
    request = _plane_request(parent, crop).with_roi_profile(
        roi, profile_axis=0
    )
