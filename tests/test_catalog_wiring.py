...
def test_multi_run_selection_accumulates_on_run_list(qapp, app_model):
    ...
    assert len(session.session.available_models) == 3
    run_list_uids = {model.uid for model in session.session.available_models}
...
def test_deselect_run_removes_run_from_presenter_run_list(qapp, app_model):
    ...
    assert len(session.session.available_models) == 2

    session.catalog.deselect_run(first.uid)

    remaining = {model.uid for model in session.session.available_models}
