...
def test_selection_override_and_clear():
    ...
    plot.set_selected_keys(x_keys, y_keys[:1], norm_keys)

    uid0, uid1 = runs[0].uid, runs[1].uid
    plot.set_selection_for(uid0, x_keys, y_keys[:1], [])
    assert plot.selection_for(uid0).norm == ()
    assert plot.selection_for(uid1).norm == tuple(norm_keys)

    plot.clear_selection_overrides()
    assert plot.selection_for(uid0).norm == tuple(norm_keys)
    assert plot.selection_for(uid1).y == tuple(y_keys[:1])
