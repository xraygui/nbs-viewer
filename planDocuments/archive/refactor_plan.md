...
| 6 | `BlueskyRun._infer_dims_from_shape` uses `range(0, ndim)` where `MemoryRun` uses `range(1, ndim)`, so every dimension receives the previous dimension's `axes` hint when `getAxisHints` is non-empty. Masked by name-list truncation. | ✅ [`data_contract_plan.md`](data_contract_plan.md) step 1 — `range(1, ndim)`, justified against a UCAL run's real tiled dims rather than against the other backends. |
| 7 | `BlueskyRun.getRunKeys` ends with `ykeys[1] = all_keys`, so rank-3 camera keys are reported as rank 1 and the two backends disagree about the grouping. | ✅ [`data_contract_plan.md`](data_contract_plan.md) step 1 — grouped by `len(getShape(key))`, which the key table already pays for. `KafkaRun` shares the defect and is left open: its `getShape` calls `getData`, which would materialize every buffered array on a live stream. |
...
| 14 | Toggling a normalization key on or off re-reads the whole N-D detector array. View-pipeline step 7 moved normalization ahead of the projection reduce and then cached the *normalized* block, which forced `norm_keys` into `RunSource._block_cache_key`. Reproduced on the VPPEM fixture: turning `i0` on reads `['PCOEdge_image', 'i0']`, turning it off reads `['PCOEdge_image']`, where a transform change correctly reads nothing. Bug 8's sibling, made by bug 8's fix, and toggling norms is routine interaction rather than an edge case. | open — [`data_contract_plan.md`](data_contract_plan.md) step 5 |
| 15 | With no X key selected, `CatalogRun.analyze_dimensions` falls back to the run's declared motors and assigns them to axes positionally, overriding the key's own correct dimension names and producing duplicate axis names. Reproduced on `image_scan_run`: `get_dims("detector_cube", [])` returns `('time','pixel','dim_2')` while `describe_axes("detector_cube", [])` returns `('pixel','pixel','dim_2')`; with an X key selected it is correct again. No X key selected is the initial state, so it is reachable by opening a run. xarray warns rather than raising on duplicate dims — "most xarray functionality is likely to fail silently" — so the contract would not catch it. Third member of bugs 6 and 7's family: a layer disagreeing with a key's own declared dimensions. | ✅ [`data_contract_plan.md`](data_contract_plan.md) step 1 — and it had a second trigger the report missed: selecting `pixel`, which is both a motor and a real dimension of the cube, produces the same duplicate. Took removing the motors fallback *and* a guard against replacing a dimension with a name another axis already holds. Separating a dimension from the coordinate plotted against it is step 8. |
| 16 | Selecting as X a key that names one of a key's detector axes other than the first reorders the axis names. `CatalogRun._analyze_dimensions` places a selected X key that is also a dimension straight after `time`, ahead of the key's other axes: for storage dims `(time, a, b)` and shape `(3, 4, 5)`, selecting `b` gives `('time', 'b', 'a')`, so storage axis 1, length 4, is labelled `b`. `KeyInfo.from_dims` checks the count and uniqueness of names, not their order, so nothing downstream sees it. Reproduced against a faked run; no shipped fixture or real run seen so far has such a key. Fourth member of bugs 6, 7 and 15's family. | open — [`data_contract_plan.md`](data_contract_plan.md) step 8, which fixes it by construction: dimensions are never reordered |
