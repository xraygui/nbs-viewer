...
| 15 | With no X key selected, `CatalogRun.analyze_dimensions` falls back to the run's declared motors and assigns them to axes positionally, overriding the key's own correct dimension names and producing duplicate axis names. Reproduced on `image_scan_run`: `get_dims("detector_cube", [])` returns `('time','pixel','dim_2')` while `describe_axes("detector_cube", [])` returns `('pixel','pixel','dim_2')`; with an X key selected it is correct again. No X key selected is the initial state, so it is reachable by opening a run. xarray warns rather than raising on duplicate dims — "most xarray functionality is likely to fail silently" — so the contract would not catch it. Third member of bugs 6 and 7's family: a layer disagreeing with a key's own declared dimensions. | open — [`data_contract_plan.md`](data_contract_plan.md) step 1 |
